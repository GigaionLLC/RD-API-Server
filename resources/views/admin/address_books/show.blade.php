...
@section('content')
            ...
            <aside class="rd-card rd-card--quiet rd-address-book__rail" aria-labelledby="address-book-tags-title">
                ...
                <div class="rd-card__body">
                    ...
                    @if ($addressBook->tags->isEmpty())
                        <p class="rd-help">No tags yet. Add one to organize larger address books.</p>
                    @endif
                </div>
            </aside>
                ...
                <div class="rd-peer-grid" id="peerGrid">
                    ...
                    @empty
                        <div class="rd-empty rd-form-grid__full">
                            ...
                            <p class="rd-empty__body">Add a remote ID to make it available to this book's owner and collaborators.</p>
    ...
    {{-- Add or edit a peer. --}}
    <div class="modal fade" id="peerModal" tabindex="-1" aria-labelledby="peerModalTitle" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" id="peerForm" class="modal-content" data-add-url="{{ route('admin.address-books.peers.store', $addressBook) }}">
                ...
                <div class="modal-header">
                    <h2 class="modal-title h5" id="peerModalTitle">Add ID</h2>
                ...
                <div class="modal-body rd-stack rd-stack--md">
                    ...
                    <div class="rd-field"><label class="rd-label" for="peerAlias">Alias</label><input class="rd-input" name="alias" id="peerAlias"></div>
                    <div class="rd-field"><label class="rd-label" for="peerNote">Note</label><input class="rd-input" name="note" id="peerNote" maxlength="300"></div>
                    <div class="rd-field"><label class="rd-label" for="peerPassword">Password <span class="rd-muted">(leave blank to keep)</span></label><input class="rd-input" id="peerPassword" type="password" name="password" autocomplete="new-password"></div>
                    <fieldset class="rd-field">
                        <legend class="rd-label">Tags</legend>
                        <div class="rd-choice-list">
                            @forelse ($addressBook->tags as $tag)
                                <label class="rd-check-card"><input type="checkbox" class="peer-tag" name="tags[]" value="{{ $tag->name }}"> <span>{{ $tag->name }}</span></label>
                            @empty
    ...
    <div class="modal fade" id="tagModal" tabindex="-1" aria-labelledby="tagModalTitle" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <form method="POST" id="tagForm" class="modal-content" data-add-url="{{ route('admin.address-books.tags.store', $addressBook) }}">
                ...
                <div class="modal-header">
                    <h2 class="modal-title h5" id="tagModalTitle">Add tag</h2>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body rd-form-grid rd-form-grid--2">
                    <div class="rd-field"><label class="rd-label" for="tagName">Name</label><input class="rd-input" name="name" id="tagName" required></div>
                    <div class="rd-field"><label class="rd-label" for="tagColor">Colour</label><input class="rd-color-input" type="color" name="color" id="tagColor" value="#1e88e5" data-default-color="#1e88e5"></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="rd-btn rd-btn--ghost" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="rd-btn rd-btn--primary">Save</button>
                    ...
                    <form method="POST" action="{{ route('admin.address-books.sharing', $addressBook) }}" class="rd-stack rd-stack--md">
                        ...
                        <div class="rd-form-grid rd-form-grid--2">
                            <div class="rd-field">
                                ...
                                <input class="rd-input" id="shareNote" name="note" value="{{ $addressBook->note }}" maxlength="255" placeholder="e.g. Front-desk machines">
                            </div>
                            <div class="rd-field">
                                <label class="rd-label" for="shareMaxPeers">Max peers</label>
                                <input class="rd-input" id="shareMaxPeers" type="number" name="max_peers" min="0" value="{{ $addressBook->max_peers }}" placeholder="Blank = server default">
                    ...
                    <section class="rd-stack rd-stack--md" aria-labelledby="collaborators-title">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="rd-table__actions">
                                                <form method="POST" action="{{ route('admin.address-books.collaborators.destroy', $collaborator) }}" class="m-0">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="rd-icon-btn rd-icon-btn--danger" aria-label="Remove {{ $collaborator->user->username ?? 'collaborator' }}" title="Remove collaborator" data-confirm="Remove {{ $collaborator->user->username ?? 'user' }}?"><i class="ri-close-line" aria-hidden="true"></i></button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <form method="POST" action="{{ route('admin.address-books.collaborators.store', $addressBook) }}" class="rd-inline-form">
                            ...
                            <div class="rd-combo rd-grow" data-url="{{ route('admin.users.search') }}">
                                ...
                                <input type="text" class="rd-input rd-combo__input" aria-label="Collaborator" placeholder="Search user…" autocomplete="off">
                                <div class="rd-combo__menu"></div>
                            </div>
                            <label class="visually-hidden" for="collaboratorRule">Permission</label>
                            <select class="rd-select rd-max-w-sm" id="collaboratorRule" name="rule">
                            ...
                            @error('user_id')<span class="rd-help rd-help--error rd-form-grid__full">{{ $message }}</span>@enderror
                        </form>
                    </section>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('admin.address-books.import', $addressBook) }}" enctype="multipart/form-data" class="modal-content">
                ...
                <div class="modal-body rd-stack rd-stack--md">
                    ...
                    <div class="rd-field">
                        ...
                        <input class="rd-input" id="peerImportFile" type="file" name="file" accept=".csv,text/csv" required>
