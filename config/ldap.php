...
return [
    ...
    // Directory server connection. LDAP_PORT applies to bare hosts; full ldap:// or ldaps://
    // URIs use an embedded port or their standard scheme port (389 or 636).
    ...
    // Transport security. Bare hosts and ldap:// endpoints are upgraded with verified StartTLS
    // by default. ldaps:// endpoints use implicit TLS and never receive a second StartTLS upgrade.
    'use_starttls' => (bool) env('LDAP_USE_STARTTLS', true),
    'tls_verify' => (bool) env('LDAP_TLS_VERIFY', true),

    // Emergency compatibility escape hatch for legacy isolated directories. This must be true
    // before plaintext LDAP or disabled certificate verification is accepted.
    'allow_insecure' => (bool) env('LDAP_ALLOW_INSECURE', false),
