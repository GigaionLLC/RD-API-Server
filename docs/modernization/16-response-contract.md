...
| 5 | `/api/login` | POST | F-obj via `user_model.login` | JSON **object**; success ⇒ AuthBody `{access_token,type:"access_token",tfa_type,secret,user:UserPayload}`; email step ⇒ `{type:"email_check",tfa_type:"email_check",secret,user}`; error ⇒ `{error}` (HTTP 200 or non-200) | `iris.Map{...}` (object) incl. `user{name,email,note,status,is_admin}` — **no `info`** | success uses `authBody()` with `user.info`; email challenge includes stock-client `type`, `secret`, and `user.name` | **OK** for Flutter. The second `type:"email_code"` request must echo the secret and same user/id/uuid; the row-local five-guess budget is independent of IP throttles. | none |
