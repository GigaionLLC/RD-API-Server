...
|---|----------|--------|---------------|------------------------|--------------------|--------------|----------|-----|
| 1 | `/api/heartbeat` | POST | R-hash | JSON **object**; optional keys `sysinfo`(any), `disconnect`([i32]), `modified_at`(i64), `strategy`({config_options,extra}) | `{modified_at, strategy}` object | matching approved identity gets policy/commands; unknown, mismatched, or unapproved identity gets `{}` | **OK.** Unknown devices fail closed by default without registration, strategy, webhook, or disconnect side effects; `{}` remains the required object shape. | none |
| 2 | `/api/sysinfo` | POST | str | plain text `"SYSINFO_UPDATED"` / `"ID_NOT_FOUND"` | `Text:"SYSINFO_UPDATED"` / `"ID_NOT_FOUND"` | matching approved identity gets `SYSINFO_UPDATED`; denied identity gets `ID_NOT_FOUND` | **OK.** Unknown devices require authenticated deployment/approval by default; explicit legacy enrollment retains the same accepted response under finite registration limits. | none |
...
| 5 | `/api/login` | POST | F-obj via `user_model.login` | JSON **object**; success ⇒ AuthBody `{access_token,type:"access_token",tfa_type,secret,user:UserPayload}`; challenge ⇒ `{type:"email_check",tfa_type:"email_check"|"tfa_check",secret,user}`; error ⇒ `{error}` (HTTP 200 or non-200) | `iris.Map{...}` (object) incl. `user{name,email,note,status,is_admin}` — **no `info`** | success uses `authBody()` with `user.info`; email/TOTP challenges include stock-client `type`, `tfa_type`, opaque `secret`, and `user.name` | **OK** for Flutter. The second `type:"email_code"` request must echo the secret and same user/id/uuid; TOTP also sends equal `verificationCode`/`tfaCode`. Both use single-use, expiring, row-locked five-guess challenges independent of IP throttles. | none |
...
| 25 | `/api/audit/conn` | POST | none (fire-and-forget) | body ignored | `mvc.Response{}` | `{}` for accepted and silently rejected events | **OK.** Writes require an existing approved device with exact `id + uuid`; malformed, oversized, or limited events no-op without changing the acknowledgement. | none |
| 26 | `/api/audit/file` | POST | none | body ignored | `mvc.Response{}` | `{}` for accepted and silently rejected events | **OK.** Same device binding and response-preserving rejection contract as row 25. | none |
| 27 | `/api/audit/alarm` | POST | none | body ignored | `{error:"11"}` (lantongxue stub) | `{}` for accepted and silently rejected events | **OK.** Same device binding and response-preserving rejection contract as row 25. | none |
| 28 | `PUT /api/audit` (note) | PUT | none (status only) | only `statusCode==200` matters (`dialog.dart:1664-1690`) | handled inside `PostAuditConn` (`note` branch) | bearer-authenticated route updates by server-issued `guid`; returns `{}` | **OK.** GUID lookup and update independently require account access to the target device; denied requests preserve the empty 200 shapes. The unauthenticated legacy POST note shape cannot prove the controlled device and no-ops unless it additionally supplies the exact UUID. | none |
| 29 | `/api/record` | POST | R-map (record_upload) | JSON **object**; bail only if `error` key present (`record_upload.rs:106-110`) | n/a | `{}` on accepted phases; `{error}` on validation, authorization, quota, or rate rejection | **OK.** The route is disabled by default and requires an explicit IP/CIDR or secret-header boundary. Non-2xx rejections also keep the `error` key so the client aborts rather than accepting a dropped chunk. | none |
